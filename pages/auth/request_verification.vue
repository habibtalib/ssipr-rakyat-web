<template>
  <div class="column is-half is-offset-one-quarter">
    <form @submit.prevent="requestVerification()">
      <div class="is-divider" :data-content="$t('l.requestVerification')"></div>
    ...
    requestVerification() {
      this.$validator.validateAll().then(result => {
        if (result) {
          ...
          this.$store
            .dispatch('applicant/request_verification', {
              email: this.email
            })
            .then(res => {
              ...
              if (res.error) {
                ...
                Dialog.alert({
                  message:
                    'Permintaan untuk menghantar semula e-mel pengesahan telah berjaya disahkan. Sila semak e-mel anda untuk langkah seterusnya.',
                  type: 'is-success',
                  hasIcon: true,
                  icon: 'check-circle',
                  iconPack: 'fa'
